...
import { TARGET_KEYS } from '../constants';
    ...
    <div className="flex flex-col items-center justify-center max-w-2xl w-full text-center space-y-12 animate-in fade-in zoom-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 p-6 rounded-xl flex flex-col items-center">
          ...
          <p className="text-sm text-slate-400 mt-1">Instant feedback on every keystroke</p>
        </div>
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 p-6 rounded-xl flex flex-col items-center">
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 p-6 rounded-xl flex flex-col items-center">
          <div className="w-8 h-8 flex items-center justify-center font-mono font-bold text-xl text-green-400 mb-3 border border-green-400/30 rounded bg-green-400/10">A</div>
      ...
      <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-800">
        <p className="text-xs font-mono text-slate-500 uppercase tracking-widest mb-2">Training Keys</p>
        <div className="flex flex-wrap justify-center gap-2">
          {TARGET_KEYS.slice(0, 10).map((k) => (
            <span key={k} className="inline-flex items-center justify-center min-w-[2rem] px-2 h-8 rounded bg-slate-800 border border-slate-700 text-slate-300 font-mono text-xs font-bold">
              {k}
            </span>
          ))}
          <span className="inline-flex items-center justify-center w-8 h-8 text-slate-600">...</span>
        </div>
      </div>
