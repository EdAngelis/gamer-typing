...
import { TARGET_KEYS, GAME_DURATION } from '../constants';
import { Score, FeedbackStatus } from '../types';
...
export const ActiveGame: React.FC<ActiveGameProps> = ({ onGameOver }) => {
  ...
  // Dynamic Styles
  const getCardStyles = () => {
    if (feedback === 'CORRECT') return 'bg-green-500 border-green-400 shadow-[0_0_50px_rgba(34,197,94,0.4)] scale-105';
    if (feedback === 'WRONG') return 'bg-red-500 border-red-400 shadow-[0_0_50px_rgba(239,68,68,0.4)] animate-shake';
  ...
  return (
    <div className="w-full max-w-2xl flex flex-col items-center space-y-12 animate-in fade-in duration-300">
      ...
      <div className="w-full flex justify-between items-center px-6 py-4 bg-slate-800/50 rounded-full border border-slate-700/50 backdrop-blur-sm">
        <div className={`flex items-center space-x-2 font-mono text-xl font-bold ${timeLeft <= 10 ? 'text-red-400 animate-pulse' : 'text-indigo-400'}`}>
      ...
      <div className="relative group">
        <div className={`
            ...
            ${getCardStyles()}
        `}>
             <span className={`${getFontSize()} font-mono font-bold leading-none select-none ${getTextColor()} text-center break-words px-2`}>
               {displayKey}
             </span>
        </div>
